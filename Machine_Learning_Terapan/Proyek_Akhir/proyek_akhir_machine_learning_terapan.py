...
def evaluate_precision_at_k_for_user(user_id, model, films_df, df, films_to_films_encoded, user_to_user_encoded, films_encoded_to_films, k=10):
    ...
    # Ambil indeks 10 rating tertinggi secara terbalik (descending)
    top_ratings_indices = np.argsort(predictions)[-k:][::-1]
    ...
    if len(liked_by_user) == 0:
        ...
        return 0.0 # Or return None, depending on how you want to handle this

    # Hitung berapa dari top-k rekomendasi yang termasuk dalam daftar liked_by_user
    hits = [film for film in recommended_films_ids if film in liked_by_user]
    precision = len(hits) / k

    print(f"User {user_id}: Recommended {k} items, {len(hits)} hits in liked items. Precision@{k}: {precision:.4f}")

    return precision

# Example Usage:
# Let's evaluate Precision@10 for the randomly selected user_id we used for recommendation
if 'user_id' in locals(): # Check if user_id was successfully selected in the previous cell
    ...
    print("Cannot perform Precision@k evaluation: No active user was selected for recommendation.")
...
if active_users:
    sample_users_for_eval = active_users[:10] # Evaluate for the first 10 active users
    precision_scores = []
    print("\n--- Evaluating Precision@10 for a sample of active users ---")
    ...
    if precision_scores:
        ...
        print("\nCould not calculate average Precision@10: No users in the sample had films with rating >= 4 or unseen films.")
else:
