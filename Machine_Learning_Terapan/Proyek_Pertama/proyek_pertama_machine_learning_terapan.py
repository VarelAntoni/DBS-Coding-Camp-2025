...
Logistic Regression mencari koefisien linier (bobot dan bias) terbaik melalui proses optimasi menggunakan algoritma seperti liblinear atau saga untuk memperkecil loss function (negative log-likelihood).
...
Dalam kasus ini digunakan kernel linier, yang berarti SVM mencoba mencari batas pemisah linier antara kelas. Untuk data non-linier, bisa digunakan kernel non-linear seperti RBF, namun tidak digunakan pada eksperimen ini.
...
XGBoost menggabungkan banyak model lemah (pohon keputusan pendek) menjadi satu model kuat, serta mendukung teknik pruning, regularisasi (untuk menghindari overfitting), dan parallel training untuk efisiensi komputasi.
